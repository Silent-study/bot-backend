...
function loadExtConfig() {
    chrome.storage.local.get(['token'], ({ token }) => {
        ...
        fetch(backendBase + '/api/config', {
            ...
            .then(r => r.json())
            .then(data => {
                renderExtConfig(data.config, data.botActive);
            })
            .catch(() => { });
    });
}
...
function renderExtConfig(cfg, botActive) {
    ...
    CONFIG_FIELDS.forEach(({ key, label }) => {
        ...
        row.innerHTML =
            ...
            '<input type="checkbox" id="ext-cfg-' + key + '"' + (cfg[key] ? ' checked' : '') + '>' +
    ...
    if (botActive) {
        ...
        document.getElementById('ext-config-actions').classList.add('ext-config-disabled');
    } else {
        lockedMsg.classList.add('hidden');
        list.classList.remove('ext-config-disabled');
        accuracyRow.classList.remove('ext-config-disabled');
        document.getElementById('ext-config-actions').classList.remove('ext-config-disabled');
    }
}
